...
story6 = load_image('Story6.png')
story7 = load_image('Story7.png')
story8 = load_image('Story8.png')
story9 = load_image('Story9.png')
story10 = load_image('Story10.png')
story11 = load_image('Story11.png')
story12 = load_image('Story12.png')

shop1 = load_image('Shop.png')
shop2 = load_image('Shop2.png')
        ...
        if (event.type,event.button) == (SDL_MOUSEBUTTONDOWN,SDL_BUTTON_LEFT):
            if(start_state == 2):
                if event.x <= 710 and event.x >= 590 and event.y >= 100 and event.y <= 130:
                    ...
                    start_state = 0
            if (start_state == 1):
                if event.x <=695 and event.x >=640 and event.y >=540 and event.y <=580:
                    story_state = 1
                if event.x <=770 and event.x >=720 and event.y >=540 and event.y <=580:
                    shop_state = 1
            if(start_state == 3):
            ...
            if (start_state == 5):
                if event.x <= 660 and event.x >= 610 and event.y >= 140 and event.y <= 180:
                    ...
                    enter()
            if (shop_state == 1):
                if event.x <=400 and event.x >=330 and event.y >=155 and event.y <=175: #선글라스
                    if(money > 300):
                        shop_state = 2
                        money -= 300
                if event.x <=220 and event.x >=120 and event.y >=120 and event.y <=170: #자동차
                    if(money > 4000):
                        shop_state = 2
                        money -= 4000
                if event.x <=690 and event.x >=535 and event.y >=115 and event.y <=175: #집
                    if(money > 8000):
                        shop_state = 2
                        money -= 8000
                if event.x <=144 and event.x >=123 and event.y >=265 and event.y <=300: #반지
                    if(money > 1500):
                        shop_state = 2
                        money -= 1500
                if event.x <=265 and event.x >=295 and event.y >=255 and event.y <=305: #펫
                    if(money > 500):
                        shop_state = 2
                        money -= 500
                if event.x <=477 and event.x >=432 and event.y >=240 and event.y <=305: #목걸이
                    if(money > 2000):
                        shop_state = 2
                        money -= 2000
                if event.x <=683 and event.x >=648 and event.y >=244 and event.y <=311: #향수
                    if(money > 500):
                        shop_state = 2
                        money -= 500
                if event.x <= 158 and event.x >= 112 and event.y >= 384 and event.y <= 438:  # 귀걸이
                    if (money > 1500):
                        shop_state = 2
                        money -= 1500
                if event.x <= 334 and event.x >= 274 and event.y >= 396 and event.y <= 440:  # 뽕
                    if (money > 3000):
                        shop_state = 2
                        money -= 3000
                if event.x <= 512 and event.x >= 459 and event.y >= 380 and event.y <= 446:  # 코트
                    if (money > 2000):
                        shop_state = 2
                        money -= 2000
                if event.x <= 670 and event.x >= 636 and event.y >= 393 and event.y <= 445:  # 핸드폰
                    if (money > 700):
                        shop_state = 2
                        money -= 700
                if event.x <=200 and event.x >=100 and event.y >=520 and event.y <=560:
                    shop_state = 0
            if (shop_state == 2):
                if event.x <=200 and event.x >=100 and event.y >=520 and event.y <=560:
                    shop_state = 0
while game_status == 1 or game_status == 0:
    ...
    if (shop_state == 1):
        shop1.draw(400,300)
        font.draw(20, 550, 'Money:%d' % money)
    if (shop_state == 2):
        shop2.draw(400,300)
    if (story_state == 1):
        story1.draw(400,300)
    ...
    if (story_state == 9):
        story9.draw(400, 300)
    if (story_state == 10):
        story10.draw(400, 300)
    if (story_state == 11):
        story11.draw(400, 300)
    if (story_state == 12):
        story12.draw(400, 300)

    if (start_state == 3):
        start3.draw(400,300)
    if (start_state == 4):
        clear.draw(400,300)
        font.draw(230,300,'Score:%d' % score)
    if (start_state == 5):
        clear2.draw(400,300)
        font.draw(230, 300, 'Score:%d' % score)
